# SOL Math Gaps App - Modeled using Grades 3-6 Math SOL Tests
# Last Updated 12/5/25
# Deployed to virginiaequitycenter.shinyapps.io/math_gaps/
...
# Read in data ----
app_data <- read_csv("math_gap_data.csv")
    ...
    ggplotly(tornado_plot, height = 2000, tooltip = "text") %>% 
      ...
      layout(xaxis = list(fixedrange = TRUE), 
             ...
             legend = list(orientation = 'h', title = FALSE,
                           ...
                           font = list(size = 14)),
             margin = list(t = 80),
             title = paste0("Gaps in Math SOL Test Pass Rates by ", input$selected_group)) %>% 
